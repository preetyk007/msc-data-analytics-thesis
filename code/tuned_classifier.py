# -*- coding: utf-8 -*-
"""
Created on Wed Dec  4 02:16:09 2019
...
def store_results(model, X_test, actual, preds):
    ...
    filename = output_path + "results_" + classifier + ".txt"
...
def build_model():
    ...
    if(classifier == 'svm'):
        model, param_grid = svm_classifier.create_model()
    else:
        model, param_grid = fc_classifier.create_model()
    
    random_search = RandomizedSearchCV(estimator=model, param_distributions=param_grid, n_iter=20, cv=10, iid=False)
    ...
    output = tuned_model.fit(X_train, Y_train)
    preds = tuned_model.predict(X_test)
    
    if(classifier == 'svm'):
        print("Training accuracy before fine_tuning: {:.2f}%\n".format(random_search.best_score_ * 100))
    ...
    store_results(tuned_model, X_test, Y_test, preds)
